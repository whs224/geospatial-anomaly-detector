...
A distributed intelligence pipeline that ingests live flight telemetry over a central-European enroute corridor (the Basel–Zürich–Munich traffic lane), detects kinematic anomalies in real time, and visualizes high-priority targets — with the evidence behind every alert — for operator review.
...
* **Ingestor Service (Python):** Polls OpenSky `/states/all` for the configured bounding box (default: a central-European enroute corridor spanning northern Switzerland, southern Germany, and the western Austrian approach). Handles rate limiting with exponential backoff and `Retry-After`, supports optional OAuth2 client credentials, filters out on-ground traffic and null coordinates, batch-inserts idempotently (`execute_values` + `ON CONFLICT DO NOTHING`), and prunes history past the retention window.
...
* **Intelligence/Detector (Python):** A decoupled worker that compares consecutive observations per aircraft inside a sliding window. An implied acceleration (velocity change divided by the time gap) above the threshold — between observations close enough in time to be comparable — is persisted to `anomaly_events` with the full evidence: speed before/after, delta, time gap, and implied acceleration in m/s². Detection stays isolated from ingestion, so a slow query never blocks data capture.
...
2.  **(Optional) Configure:**
    ...
    cp .env.example .env
    ```
    Everything runs with defaults, but OpenSky's anonymous tier only grants ~400 API credits/day (the default corridor bounding box costs 1 credit per request). A free OpenSky account gets 4,000/day — create an API client under *Account → API Client* and put the credentials in `.env`.
...
* **Physics-aware thresholds:** A raw velocity delta means nothing without the time span it happened over, so the detector thresholds on *implied acceleration* (delta-v over the time gap), not the bare delta — a gradual enroute speed change no longer fires, only a physically abrupt one. Pairs further apart than the max-gap window are never compared (an aircraft leaving and re-entering coverage isn't an anomaly), and the implied acceleration is recorded with every event.
