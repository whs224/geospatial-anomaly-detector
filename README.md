...
3. **Detects** kinematic outliers — aircraft whose implied acceleration exceeds a configurable threshold — as a first-pass filter for unusual motion or data-quality issues, and persists the kinematic evidence, not just a flag.
4. **Classifies** each aircraft into an airspace sector with a GiST-indexed spatial query, and visualizes the result.
...
- **Credit math.** OpenSky charges API credits by requested area: ~40 sq deg falls in the 25–100 tier = **2 credits/request**. At 10s that is 360 requests/hour = **720 credits/hour**. A full authenticated day (~4,000 credits) lasts ~5.5 hours of continuous polling before throttling; anonymous (~400) lasts ~33 minutes.
...
The API classifies each aircraft into an airspace sector using a GiST-indexed `ST_Contains` point-in-polygon predicate, evaluated in PostgreSQL.

**1. The sectors.** `migrations.sql` seeds a `sectors` table with **four** polygons, each an octagon (8 distinct vertices plus the closing point). They are approximations of the real TRACON (Terminal Radar Approach Control) airspaces — octagonal shapes over the airport clusters, not exact FAA boundaries:
...
**2. The predicate.** `api.py`'s `LATEST_POSITIONS_QUERY` takes the latest position per active aircraft, then `LEFT JOIN LATERAL` against `sectors` on **`ST_Contains(sectors.geom, aircraft.geom)`** — a point-in-polygon test — keeping one sector (`ORDER BY s.id LIMIT 1`). `LEFT JOIN` so an aircraft in no sector comes back with `NULL` (rendered as "En route"). The result is exposed as `sector`/`sector_code` in each `/flights` feature; a separate `/sectors` endpoint returns the polygons themselves (via `ST_AsGeoJSON`) for the map overlay.

**3. Why GiST, and the two-phase behavior.** `ST_Contains` is evaluated in two stages, and the GiST index accelerates the first:
- **Stage 1 (index-served):** the GiST tree stores each polygon's bounding box; it finds sectors whose bbox could contain the point (the `geom ~ point` operator), discarding sectors that obviously can't match.
- **Stage 2 (exact):** the precise point-in-polygon geometry runs only on that small candidate set.

**4. Query plan.** `EXPLAIN ANALYZE` shows the GiST index in use:
...
- **Evidence, not a flag.** `anomaly.py::build_anomaly_records` computes the full evidence (speed before/after, delta_v, time_gap, implied_accel) and `persist_anomalies` writes it to `anomaly_events`. The API reads this evidence rather than recomputing it, so there is one source of truth and alerts survive restarts. The popup one-liner (`format_summary`) reads e.g. *"Implied acceleration 3.4 m/s² over 8s exceeds the 2.0 m/s² threshold."*

## ⚙️ The concurrency model
The four services use different concurrency models:

- **detector — asyncio event loop.** `detector.py` runs on an asyncio event loop (`asyncio.run(_amain())`) using the async psycopg (psycopg3) driver. The cycle `await`s the candidate query and the anomaly writes (`await cursor.execute/fetchall/fetchone`, `await conn.commit()`), the inter-cycle wait is an awaited, interruptible sleep rather than `time.sleep`, and SIGTERM is handled with `loop.add_signal_handler` so `docker stop` drains the current cycle and exits cleanly. Startup (`wait_for_db`, advisory-lock migrations) also runs over the async driver. The loop runs one detection query per cycle, so the async model provides non-blocking I/O rather than overlapping concurrent work.
- **ingestor — synchronous worker.** `main.py` is a plain `while True` loop with blocking `psycopg2` calls and a blocking heartbeat-sleep — a synchronous, decoupled worker in its own process/container.
- **api — sync handlers on an async server.** FastAPI runs on the uvicorn ASGI server, but the route handlers are defined with `def` (not `async def`) and use the synchronous `psycopg2` connection pool (`db.create_pool`, min 1 / max 5). FastAPI runs each handler in a threadpool, so request concurrency comes from the threadpool and pooled connections rather than from async DB calls.
- **db** is the PostGIS server process.

The services share nothing but the database, so a slow or hung query in one does not block another — they are separate processes.

## 🩺 Health, lifecycle, and containers
- **Loop healthchecks.** `ingestor` and `detector` touch `/tmp/heartbeat` every cycle; the healthcheck (`find /tmp/heartbeat -newermt '-190 seconds'`) marks the container unhealthy if the file is older than 190s.
- **API healthcheck.** Hits `/health`, which runs `SELECT 1` through the pool.
- **Graceful shutdown.** The ingestor uses a SIGTERM→`SystemExit` handler; the detector uses asyncio signal handling. Both exit cleanly on `docker stop`.
...
pytest
```
`tests/test_parsing.py` (10 cases) covers `parse_flight_state`; `tests/test_anomaly.py` (7 cases) covers `build_anomaly_records` and `format_summary`. These are pure-function unit tests that run without a database.
